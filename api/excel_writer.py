...
def cleanup_previous_output_files(script_dir):
    ...
    used by excel_writer.py: output_{timestamp}.xlsx
    where timestamp format is YYYY-MM-DD_HH-MM-SS.

    Files from other scripts are not affected as they use different naming patterns.

    Args:
        script_dir: Path to the script directory (str or Path)
    """
    # Convert to Path if needed
    script_path = Path(script_dir)

    # Pattern: output_{YYYY-MM-DD_HH-MM-SS}.xlsx
    pattern = re.compile(r'^output_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.xlsx$')

    deleted_count = 0
    # Find all matching files in the script directory
    for file_path in script_path.glob('output_*.xlsx'):
